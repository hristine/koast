...
function removeBackup(backupRecord, mongoUri) {
  var receipts = backupRecord.receipts;
  var handler = getHandler(backupRecord.type);
  var promises = receipts.map(function (receipt) {
    handler.destroy(receipt.data);
  });
  return q.all(promises).thenResolve('done');
}
