  ...
  MongoClient.connect(conf.database.url, function(err, db) {
    ...
    api.register('backup', 'list', 'GET', function() {
        ...
        return q.nbind(results.toArray, results)()
          .then(function (arr) {
            // Augment backups that have a status.
            return arr.map(function (backupRecord) {
              if (liveBackups[backupRecord.backupId]) {
                backupRecord.status = liveBackups[backupRecord.backupId].status;
              }
              return backupRecord
            });
          });
    ...
    api.register('backup', 'details', 'GET', ['id'], function(id) {
      return q.nbind(backupCollection.findOne, backupCollection)({ backupId: id })
        .then(function(r) {
          if (liveBackups[r.backupId]) {
            r.status = liveBackups[r.backupId].status;
          }
          return q(r);
        });
    });
    ...
    api.register('backup', 'remove', 'POST', function(args) {
      var id = args.id;
      return q.nbind(backupCollection.findOne, backupCollection)({ backupId: id })
        .then(function(backupRecord) {
          if(!backupRecord) {
          ...
          backup.remove(backupRecord)
            .then(function () {
              backupCollection.remove(backupRecord, function (e) {
                log.error(e);
              });
            })
            .then(null, log.error.bind(log));
